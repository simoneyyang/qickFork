...
//
// >>> QickEmu-driven testbench — Verilator/open-source emulation compatible.
...
// NOTE: these values were set to 1 in emulator to shorten time 
// during testing and ensure all axi transactions are read
...
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   ...
   axis_qick_processor # (
      ...
      .IO_CTRL             (  `IO_CTRL          ) ,
      .DEBUG               (  `DEBUG            ) ,
      .QNET                (  `TNET             ) , //idk why Corey changed this?
      .QCOM                (  `QCOM             ) ,
...
// NOTE: diff from Vivado emu harness but hopefully no issues
   ...
   axis_cdcsync_v1 #(
      ...
      .B                         (168),   // Number of data bits.
// ++++++++++++ ADD EMULATOR PARAMETER
      .EMULATOR                  (EMULATOR) // this could be a problem in vivado
// ++++++++++++
   )
   u_axis_cdcsync_v1 (
      // S_AXIS for input data.
...
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   ...
   // ============
   axi_mst_avg_agent.write(AVG_DR_LEN_REG, prot, 32'h00000001, 8'hFF, resp);
   // >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
   @(posedge s_ps_dma_aclk); #0.1;
   // <<<<<<<<<<<< XILINX VIP PACKAGES
   ...
   // ============
   axi_mst_avg_agent.write(BUF_DR_LEN_REG, prot, 32'h0000039A, 8'hFF, resp);
   // >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
   @(posedge s_ps_dma_aclk); #0.1;
   // <<<<<<<<<<<< XILINX VIP PACKAGES
...
// <<<<<<<<<<<< XILINX VIP PACKAGES
// TODO: Comment this out (can't in meld)
   // // Create agents.
   // axi_mst_tproc_agent  = new("axi_mst_tproc VIP Agent",tb_qick_harness.u_axi_mst_tproc_0.inst.IF);
   // // Set tag for agents.
   // axi_mst_tproc_agent.set_agent_tag("axi_mst_tproc VIP");
   // // Start agents.
   // axi_mst_tproc_agent.start_master();
   ...
   // // Create agents.
   // axi_mst_sg_agent   = new("axi_mst_sg_0 VIP Agent",tb_qick_harness.u_axi_mst_sg_0.inst.IF);
   // // Set tag for agents.
   // axi_mst_sg_agent.set_agent_tag("axi_mst_sg_0 VIP");
   // // Start agents.
   // axi_mst_sg_agent.start_master();
   ...
   // // Create agents.
   // axi_mst_avg_agent   = new("axi_mst_avg_0 VIP Agent",tb_qick_harness.u_axi_mst_avg_0.inst.IF);
   // // Set tag for agents.
   // axi_mst_avg_agent.set_agent_tag("axi_mst_avg_0 VIP");
   // // Start agents.
   // axi_mst_avg_agent.start_master();
   ...
   // // Create agents.
   // axi_mst_qemu_agent   = new("axi_mst_qemu_0 VIP Agent",tb_qick_harness.u_axi_mst_qemu_0.inst.IF);
   // // Set tag for agents.
   // axi_mst_qemu_agent.set_agent_tag("axi_mst_qemu_0 VIP");
   // // Start agents.
   // axi_mst_qemu_agent.start_master();
...
// NOTE: is this a questionable area? will see in Vivado
...
task route_and_write(integer addr, integer data);
   ...
   if (addr >= TPROC_BASE && addr < TPROC_BASE + 32'h10000) begin
      ...
      // ============
      axi_mst_tproc_agent.write(offset, prot, data, 8'hFF, resp);
      // >>>>>>>>>>>> PULP PLATFORM AXI VIP
      ...
      // ============
      axi_mst_sg_agent.write(offset, prot, data, 8'hFF, resp);
      // >>>>>>>>>>>> PULP PLATFORM AXI VIP
   end
      ...
      // ============
      axi_mst_avg_agent.write(offset, prot, data, 8'hFF, resp);
      // >>>>>>>>>>>> PULP PLATFORM AXI VIP
   end
   else begin
