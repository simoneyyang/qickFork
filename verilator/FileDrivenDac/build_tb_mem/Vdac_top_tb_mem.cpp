...
void Vdac_top_tb_mem::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vdac_top_tb_mem::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vdac_top_tb_mem::traceConfig() const {
...
void Vdac_top_tb_mem___024root__trace_decl_types(VerilatedFst* tracep);

void Vdac_top_tb_mem___024root__trace_init_top(Vdac_top_tb_mem___024root* vlSelf, VerilatedFst* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedFst* tracep, uint32_t code) {
    ...
    Vdac_top_tb_mem___024root__trace_decl_types(tracep);
    Vdac_top_tb_mem___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vdac_top_tb_mem___024root__trace_register(Vdac_top_tb_mem___024root* vlSelf, VerilatedFst* tracep);

VL_ATTR_COLD void Vdac_top_tb_mem::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedFstC* const stfp = dynamic_cast<VerilatedFstC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vdac_top_tb_mem::trace()' called on non-VerilatedFstC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vdac_top_tb_mem___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
